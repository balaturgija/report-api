...
export class AuthService {
    ...
    async signUp({email, password, name, phone}: SignupParams, userType: UserType) {
        const userExists = await this.prismaService.user.findUnique({ where: {email: email}});
        if(userExists) throw new ConflictException();

        const hashedPassword = await bcrypt.hash(password, 10);
        const user = await this.prismaService.user.create({ data: {
            email, name, phone, password: hashedPassword, userType: userType
        }});
    ...
    private async generateJwt(name: string, id: number) {
        return await jwt.sign({name: name, id: id}, process.env.JWT_SECRET, { expiresIn: 3600});
